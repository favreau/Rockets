...
{
    F::server.handleText(
        [&](const ws::Request& request)
        {
            F::receivedMessage1 = (request.message == "hello");
            return "server";
        });
    F::client1.handleText(
        [&](const ws::Request& request)
        {
